...
class BasicInMemoryTableCatalog extends TableCatalog {
  ...
  override def createTable(
    ...
    val table = new InMemoryColumnarTable(s"$name.${ident.quoted}", schema, partitions, properties)
  ...
  override def alterTable(ident: Identifier, changes: TableChange*): Table = {
    val table = loadTable(ident).asInstanceOf[InMemoryColumnarTable]
    ...
    val newTable = new InMemoryColumnarTable(table.name, schema, table.partitioning, properties)
      .withData(table.dataMap.toMap)
  ...
  override def dropTable(ident: Identifier): Boolean = {
    Option {
      val tb = tables.remove(ident)
      tb match {
        case t: AutoCloseable =>
          t.close()
          tb
        case _ => tb
      }
